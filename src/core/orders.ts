...
function orderTotal(items: Array<{ quantity: Prisma.Decimal | number | string; unitPrice: Prisma.Decimal | number | string }>) { return roundMoney(items.reduce((sum, item) => sum.add(decimal(item.quantity).mul(decimal(item.unitPrice))), new Prisma.Decimal(0))); }

export async function createOrder(input: { branchId: string; customerId?: string; channel: OrderChannelInput; requestedAt?: string | Date; items: OrderItemInput[]; actorId?: string }) {
  ...
  const branch = await prisma.branch.findUnique({ where: { id: input.branchId }, select: { id: true, organizationId: true } });
  ...
  if (input.customerId) {
    const customer = await prisma.customer.findUnique({ where: { id: input.customerId }, select: { id: true, organizationId: true } });
    if (!customer || customer.organizationId !== branch.organizationId) throw new Error("CUSTOMER_INVALID");
  }
  const preparedItems: Array<{ productId: string; quantity: Prisma.Decimal; unitPrice: Prisma.Decimal }> = [];
  for (const item of input.items) {
    ...
    const product = await prisma.product.findUnique({ where: { id: item.productId }, select: { id: true, organizationId: true, status: true, publicPrice: true, branchProducts: { where: { branchId: input.branchId }, select: { isEnabled: true } }, prices: { where: { branchId: input.branchId, effectiveAt: { lte: requestedAt } }, orderBy: { effectiveAt: "desc" }, take: 1, select: { price: true } } } });
    ...
    preparedItems.push({ productId: product.id, quantity, unitPrice: positiveDecimal(configuredPrice.toString(), "ORDER_PRICE_INVALID") });
  }
  const total = orderTotal(preparedItems); const orderId = randomUUID();
  const order = await prisma.$transaction(async (tx) => {
    const created = await tx.order.create({ data: { id: orderId, branchId: input.branchId, customerId: input.customerId || null, channel: input.channel, status: "RECEIVED", requestedAt, items: { create: preparedItems.map((item) => ({ productId: item.productId, quantity: item.quantity, unitPrice: item.unitPrice })) } }, include: { items: true } });
    await tx.auditLog.create({ data: { organizationId: branch.organizationId, branchId: input.branchId, userId: input.actorId || null, action: "ORDER_RECEIVED", entityType: "Order", entityId: created.id, beforeData: Prisma.JsonNull, afterData: { orderId: created.id, channel: input.channel, customerId: input.customerId || null, total: total.toFixed(2) } } });
    return created;
  });
  return { ...order, total: total.toFixed(2) };
}

export async function listOrders(input: { branchId: string; status?: OrderStatusInput; limit?: number }) {
...
export async function attachSaleToOrder(input: { branchId: string; orderId: string; saleId: string }) {
  ...
  const [order, sale] = await Promise.all([prisma.order.findUnique({ where: { id: input.orderId }, select: { id: true, branchId: true, status: true, saleId: true } }), prisma.sale.findUnique({ where: { id: input.saleId }, select: { id: true, branchId: true, status: true, customerId: true } })]);
  if (!order || order.branchId !== input.branchId) throw new Error("ORDER_NOT_FOUND");
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const branch = await tx.branch.findUnique({ where: { id: input.branchId }, select: { organizationId: true } });
    if (branch) await tx.auditLog.create({ data: { organizationId: branch.organizationId, branchId: input.branchId, action: "ORDER_LINKED_TO_SALE", entityType: "Order", entityId: order.id, beforeData: { saleId: order.saleId }, afterData: { saleId: sale.id } } });
    return updated;
  });
}
