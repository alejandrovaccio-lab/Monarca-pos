...
export async function createCustomer(input: CustomerInput) {
  ...
  const phone = clean(input.phone); const email = clean(input.email)?.toLowerCase(); const taxId = clean(input.taxId)?.toUpperCase(); await ensureUniquePhone(branch.organizationId, phone);
  const customer = await prisma.$transaction(async (tx) => { const created = await tx.customer.create({ data: { organizationId: branch.organizationId, name, phone: phone ?? null, email: email ?? null, taxId: taxId ?? null } }); await tx.auditLog.create({ data: { organizationId: branch.organizationId, branchId: input.branchId, userId: input.actorId, action: "CUSTOMER_CREATED", entityType: "Customer", entityId: created.id, beforeData: Prisma.JsonNull, afterData: { name: created.name, phone: created.phone, email: created.email, taxId: created.taxId, membershipCode: membershipCode(created.id) } } }); return created; });
...
export async function updateCustomer(input: CustomerInput & { customerId: string }) {
  const { branch } = await branchContext(input.branchId, input.actorId); const current = await prisma.customer.findUnique({ where: { id: input.customerId } }); if (!current || current.organizationId !== branch.organizationId) throw new Error("CUSTOMER_NOT_FOUND");
  const name = clean(input.name); if (!name) throw new Error("CUSTOMER_NAME_REQUIRED"); const phone = clean(input.phone); const email = clean(input.email)?.toLowerCase(); const taxId = clean(input.taxId)?.toUpperCase(); await ensureUniquePhone(branch.organizationId, phone, input.customerId);
  const updated = await prisma.$transaction(async (tx) => { const result = await tx.customer.update({ where: { id: input.customerId }, data: { name, phone: phone ?? null, email: email ?? null, taxId: taxId ?? null } }); await tx.auditLog.create({ data: { organizationId: branch.organizationId, branchId: input.branchId, userId: input.actorId, action: "CUSTOMER_UPDATED", entityType: "Customer", entityId: result.id, beforeData: { name: current.name, phone: current.phone, email: current.email, taxId: current.taxId }, afterData: { name: result.name, phone: result.phone, email: result.email, taxId: result.taxId, membershipCode: membershipCode(result.id) } } }); return result; });
