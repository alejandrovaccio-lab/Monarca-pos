...
function setupContext() {
  ...
  db.product.findUnique.mockResolvedValue({ id: "product-1", organizationId: "org-1", name: "Manzana", status: "ACTIVE", publicPrice: 10, branchProducts: [{ isEnabled: true }], prices: [], costs: [{ cost: 5 }] });
}
...
describe("sale creation", () => {
  it("creates a paid sale, decrements inventory and records an item-scoped sale movement", async () => {
    setupContext(); const inventoryUpdate=vi.fn().mockResolvedValue({count:1}); const movementCreate=vi.fn().mockResolvedValue({id:"movement-1"}); const saleCreate=vi.fn().mockResolvedValue({id:"sale-1",folio:"V-TEST",status:"COMPLETED",items:[],payments:[]}); const auditCreate=vi.fn().mockResolvedValue({id:"audit-1"}); const branchProductFindUnique=branchProductMock();
