...
describe("purchase receipts", () => {
  ...
  it("rejects execution when the latest persisted approval is not APPROVED, preventing reuse of a stale approval", async () => {
    canApproveAuthorization.mockResolvedValue(true); const authorization = { id: "auth-stale-approval", status: "APPROVED", organizationId: "org-1", branchId: "branch-1", requestedById: "requester-1", type: "OTHER", entityType: "Purchase", entityId: "purchase-stale-approval", reason: "Resurtido", beforeData: null, integrityHash: "test-integrity-hash", requestedData: { purchaseId: "purchase-stale-approval", branchId: "branch-1", supplierId: "supplier-1", folio: "FAC-106", employeeId: "emp-1", purchasedAt: new Date().toISOString(), items: [{ productId: "product-1", quantity: 1, unitCost: 10 }] } };
    db.authorizationRequest.findUnique.mockResolvedValue(authorization); const findFirst = vi.fn().mockResolvedValue({ id: "approval-latest-rejected", approverId: "user-1", decision: "REJECTED", approvedAt: new Date().toISOString() }); const tx = { authorizationRequest: { findUnique: vi.fn().mockResolvedValue(authorization) }, user: { findUnique: vi.fn().mockResolvedValue({ organizationId: "org-1", status: "ACTIVE", branchAccess: [{ branchId: "branch-1" }], roles: [{ role: { name: "GERENTE" } }] }) }, authorizationApproval: { findFirst } }; db.$transaction.mockImplementation(async (callback: any) => callback(tx));
    ...
    expect(findFirst).toHaveBeenCalledWith({ where: { authorizationRequestId: authorization.id }, orderBy: { approvedAt: "desc" }, select: { id: true, approverId: true, decision: true, approvedAt: true } });
  });

  it("rejects execution when an approval predates the authorization request", async () => {
    canApproveAuthorization.mockResolvedValue(true); const requestedAt = new Date("2026-01-02T10:00:00.000Z"); const authorization = { id: "auth-approval-before-request", status: "APPROVED", organizationId: "org-1", branchId: "branch-1", requestedById: "requester-1", type: "OTHER", entityType: "Purchase", entityId: "purchase-approval-before-request", reason: "Resurtido", beforeData: null, integrityHash: "test-integrity-hash", requestedAt, resolvedAt: new Date("2026-01-02T12:00:00.000Z"), requestedData: { purchaseId: "purchase-approval-before-request", branchId: "branch-1", supplierId: "supplier-1", folio: "FAC-107", employeeId: "emp-1", purchasedAt: requestedAt.toISOString(), items: [{ productId: "product-1", quantity: 1, unitCost: 10 }] } };
    db.authorizationRequest.findUnique.mockResolvedValue(authorization); const tx = { authorizationRequest: { findUnique: vi.fn().mockResolvedValue(authorization) }, user: { findUnique: vi.fn().mockResolvedValue({ organizationId: "org-1", status: "ACTIVE", branchAccess: [{ branchId: "branch-1" }], roles: [{ role: { name: "GERENTE" } }] }) }, authorizationApproval: { findFirst: vi.fn().mockResolvedValue({ id: "approval-old", approverId: "user-1", decision: "APPROVED", approvedAt: new Date("2026-01-02T09:59:59.000Z") }) } }; db.$transaction.mockImplementation(async (callback: any) => callback(tx));
    await expect(executeApprovedPurchaseReceipt({ requestId: authorization.id, executorId: "user-1" })).rejects.toThrow("AUTHORIZATION_INTEGRITY_VIOLATION");
  });

  it("rejects execution when approval occurs after authorization resolution", async () => {
    canApproveAuthorization.mockResolvedValue(true); const requestedAt = new Date("2026-01-02T10:00:00.000Z"); const resolvedAt = new Date("2026-01-02T12:00:00.000Z"); const authorization = { id: "auth-approval-after-resolution", status: "APPROVED", organizationId: "org-1", branchId: "branch-1", requestedById: "requester-1", type: "OTHER", entityType: "Purchase", entityId: "purchase-approval-after-resolution", reason: "Resurtido", beforeData: null, integrityHash: "test-integrity-hash", requestedAt, resolvedAt, requestedData: { purchaseId: "purchase-approval-after-resolution", branchId: "branch-1", supplierId: "supplier-1", folio: "FAC-108", employeeId: "emp-1", purchasedAt: requestedAt.toISOString(), items: [{ productId: "product-1", quantity: 1, unitCost: 10 }] } };
    db.authorizationRequest.findUnique.mockResolvedValue(authorization); const tx = { authorizationRequest: { findUnique: vi.fn().mockResolvedValue(authorization) }, user: { findUnique: vi.fn().mockResolvedValue({ organizationId: "org-1", status: "ACTIVE", branchAccess: [{ branchId: "branch-1" }], roles: [{ role: { name: "GERENTE" } }] }) }, authorizationApproval: { findFirst: vi.fn().mockResolvedValue({ id: "approval-future", approverId: "user-1", decision: "APPROVED", approvedAt: new Date("2026-01-02T12:00:01.000Z") }) } }; db.$transaction.mockImplementation(async (callback: any) => callback(tx));
    await expect(executeApprovedPurchaseReceipt({ requestId: authorization.id, executorId: "user-1" })).rejects.toThrow("AUTHORIZATION_INTEGRITY_VIOLATION");
  });

  it("allows execution when the executor retains an approver role and approval provenance is valid", async () => {
    canApproveAuthorization.mockResolvedValue(true); const authorization = { id: "auth-role-valid", status: "APPROVED", organizationId: "org-1", branchId: "branch-1", requestedById: "requester-1", type: "OTHER", entityType: "Purchase", entityId: "purchase-role-valid", reason: "Resurtido", beforeData: null, integrityHash: "test-integrity-hash", requestedAt: new Date("2026-01-02T10:00:00.000Z"), resolvedAt: new Date("2026-01-02T12:00:00.000Z"), requestedData: { purchaseId: "purchase-role-valid", branchId: "branch-1", supplierId: "supplier-1", folio: "FAC-102", employeeId: "emp-1", purchasedAt: new Date("2026-01-02T10:00:00.000Z").toISOString(), items: [{ productId: "product-1", quantity: 1, unitCost: 10 }] } };
    db.authorizationRequest.findUnique.mockResolvedValue(authorization); const tx = { authorizationRequest: { findUnique: vi.fn().mockResolvedValue(authorization) }, user: { findUnique: vi.fn().mockResolvedValue({ organizationId: "org-1", status: "ACTIVE", branchAccess: [{ branchId: "branch-1" }], roles: [{ role: { name: "GERENTE" } }] }) }, authorizationApproval: { findFirst: vi.fn().mockResolvedValue({ id: "approval-1", approverId: "user-1", decision: "APPROVED", approvedAt: new Date("2026-01-02T11:00:00.000Z") }) }, purchase: { findUnique: vi.fn().mockResolvedValue({ id: "existing-purchase" }) } }; db.$transaction.mockImplementation(async (callback: any) => callback(tx));
