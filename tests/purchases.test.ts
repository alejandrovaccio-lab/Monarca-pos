...
describe("purchase receipts", () => {
  ...
  it("rejects execution when the latest persisted approval is not APPROVED, preventing reuse of a stale approval", async () => {
    ...
    db.authorizationRequest.findUnique.mockResolvedValue(authorization); const findFirst = vi.fn().mockResolvedValue({ id: "approval-latest-rejected", approverId: "user-1", decision: "REJECTED" }); const tx = { authorizationRequest: { findUnique: vi.fn().mockResolvedValue(authorization) }, user: { findUnique: vi.fn().mockResolvedValue({ organizationId: "org-1", status: "ACTIVE", branchAccess: [{ branchId: "branch-1" }], roles: [{ role: { name: "GERENTE" } }] }) }, authorizationApproval: { findFirst } }; db.$transaction.mockImplementation(async (callback: any) => callback(tx));
    ...
    expect(findFirst).toHaveBeenCalledWith({ where: { authorizationRequestId: authorization.id }, orderBy: { approvedAt: "desc" }, select: { id: true, approverId: true, decision: true } });
  });
