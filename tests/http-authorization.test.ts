...
const { requireBranchSession, requireBranchAuthorizationApprover, postAuthorizationRequest, postAuthorizationDecision } = vi.hoisted(() => ({
...
describe("HTTP authorization boundary", () => {
  ...
  it("derives requester, organization and branch from the authenticated session", async () => {
    ...
    await handleRequest(request("POST", "/authorizations", {
      branchId: "branch-1",
    ...
    expect(postAuthorizationRequest).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("derives the approver identity from the authenticated session", async () => {
    ...
    await handleRequest(request("POST", "/authorizations/auth-1/decision", {
      branchId: "branch-1",
      approverId: "attacker",
      decision: "APPROVED",
      notes: "approved",
    ...
    expect(postAuthorizationDecision).toHaveBeenCalledWith({
      requestId: "auth-1",
